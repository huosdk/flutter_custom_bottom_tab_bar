 ...
 */
  ...
  Path getOuterPath(Rect rect, {TextDirection? textDirection}) {
    ...
    Path _getLeftTopPath(Rect rect) {
      return new Path()
        ..moveTo(rect.left, rect.bottom - (bottomLeftRadius ?? 0))
        ..lineTo(rect.left, rect.top + (topLeftRadius ?? 0))
        ..arcToPoint(Offset(rect.left + (topLeftRadius ?? 0), rect.top),
            radius: new Radius.circular((topLeftRadius ?? 0)))
        ..lineTo(rect.right - (topRightRadius ?? 0), rect.top)
        ..arcToPoint(Offset(rect.right, rect.top + (topRightRadius ?? 0)),
            radius: new Radius.circular(topRightRadius ?? 0), clockwise: true);
    }

    Path _getBottomRightPath(Rect rect) {
      return new Path()
        ..moveTo(rect.left + (bottomLeftRadius ?? 0), rect.bottom)
        ..lineTo(rect.right - (bottomRightRadius ?? 0), rect.bottom)
        ..arcToPoint(Offset(rect.right, rect.bottom - (bottomRightRadius ?? 0)),
            radius: new Radius.circular(bottomRightRadius ?? 0), clockwise: false)
        ..lineTo(rect.right, rect.top + (topRightRadius ?? 0))
        ..arcToPoint(Offset(rect.right - (topRightRadius ?? 0), rect.top),
            radius: new Radius.circular(topRightRadius ?? 0), clockwise: false);
    }
